...
export default function SidePanel() {
    ...
    return (
        <div id="side-panel" className={`panel ${isOpen ? "side-panel-open" : "side-panel-closed"}`} >
            <button type="button" aria-label="Toggle side bar" id="sidePanelToggleIcon"
                className={`toggle-icon ${isOpen ? "open" : "closed"}`} onClick={() => toggle(!isOpen)}>
            ...
            <SidePanelContent headingClick={() => toggle(!isOpen)} />
        </div >
    );
}
